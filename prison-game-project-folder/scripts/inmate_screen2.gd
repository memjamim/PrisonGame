...
func _generate_board() -> void:
	...
	all_words.append_array(GameState.GOOD_WORDS_2)
	all_words.append_array(GameState.EVIL_WORDS_2)
...
func _try_register_word(word: String) -> bool:
	if word in GameState.GOOD_WORDS_2 and not (word in GameState.found_good):
		GameState.found_good.append(word)
		_mark_current_path_found("good")
		status_label.text = "Word found."
		_after_word_found("good")
		return true
	if word in GameState.EVIL_WORDS_2 and not (word in GameState.found_evil):
...
func _on_submit_guard_phrase_pressed() -> void:
	...
	if GameState.try_confirm_dual_inmate_reply(entered):
		...
		Dialogue.play(INMATE_DIALOGUE, GUARD_NULL, func():
